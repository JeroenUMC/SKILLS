...
### Context hygiene
   ...
   Keep steps 1–3 in **one unbroken context window** — don't compact or clear until after `/to-tickets` — so the grilling, spec, and tickets all build on the same thinking. Each `/orchestrator-implement-issue` then starts fresh, working from the ticket.
...
- **A huge, foggy effort — a greenfield project or a huge feature build, too big for one session** → **`/wayfinder`**, the most cognitively demanding flow here. When the way from here to the destination isn't visible yet, it charts a **shared map** of **decision tickets** on the issue tracker and resolves them one at a time — producing **decisions, not deliverables** — until the fog is pushed back and the way is clear. Where **`/grill-with-docs`** sharpens an idea you can hold in one session, wayfinder is for the idea you can't — and it's slower and denser, so save it for exactly that, never a well-scoped feature.
   ...
   When the map clears, **it hands off, it doesn't build**: merge onto the main flow at **`/to-spec`**, which collapses the map's linked decisions into a buildable plan, then `/to-tickets` and `/orchestrator-implement-issue` as usual. Looping the map straight into `/orchestrator-implement-issue` skips that collapse and throws the linked detail away — go straight to `/orchestrator-implement-issue` only when the effort turned out genuinely small.
...
- **`/paper-trail`** — someone doubts that a finished milestone, epic, or issue set really did what it promised, and the only answer is dozens of issues and PRs. It fixes the **claim** the set made and the **question** the reader is actually asking, then **prosecutes** the claim: grading every acceptance criterion on evidence found in PRs and committed artifacts — **a tick is never evidence** — and building the strongest case *against* before delivering a verdict. Every gap it couldn't close is printed as a **blind spot**. Out come a versioned Markdown audit and a shareable HTML artifact. Where `/changelog` asks "what's new in this window", paper-trail asks "did the promise hold" — the same PRs, a different axis.
